...
function agregarDes(pos,x,y,texto,textoM,textoP){
    ...
    fondoBlanco.setAttribute("x", x-22+8);
    fondoBlanco.setAttribute("y", y-41+2);
    fondoBlanco.setAttribute("width", "3%");
    fondoBlanco.setAttribute("height", "2.4%");
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    .then(([dataM, dataP, dataG]) => {
        ...
        if(diaSemana === 0 || diaSemana === 6 || horaC < 7 || horaC > 22) {
            ...
            for (let i = 0; i < 13; i++) {
                let elemento = dataG[i + 45][horaVi][diaSemana - 1];
                let elementoM = dataM[i + 45][horaVi][diaSemana - 1];
                let elementoP = dataP[i + 45][horaVi][diaSemana - 1];
